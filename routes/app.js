...
router.get('/trades', isLoggedIn, (req, res) => {
	Trade.find({
		_user2: req.user._id,
		isApproved: false
	}).populate('_book1 _book2 _user1 _user2').exec((err, trades) => {
		if (err) throw err;
		res.render('trades.html', {
			trades
		});
	});
});
...
router.get('/trade/accept/:tradeId', isLoggedIn, (req, res) => {
	Trade.findByIdAndUpdate({
		...
		if (err) throw err;
		Book.findByIdAndUpdate(trade._book1, {
			_user: trade._user2
		}, (err, result) => {
			if (err) throw err;
			Book.findByIdAndUpdate(trade._book2, {
				_user: trade._user1
			}, (err, result) => {
				if (err) throw err;
				res.redirect('/trades');
